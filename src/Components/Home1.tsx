// /* eslint-disable @typescript-eslint/no-redeclare */
// /* eslint-disable @typescript-eslint/no-unused-vars */
// /* eslint-disable jsx-a11y/img-redundant-alt */
// import React, { useContext } from 'react';
// import { Cards } from './Cards';
// import Nav from './Nav';
// import './Home1.css';
// import NavLink from 'react-bootstrap/esm/NavLink';
// import Cart from './Cart';
// import { Link } from 'react-router-dom';
// import { shoppingcart1,shoppingcart2,shoppingcart3} from './data'; 




// const Home=()=> {

  
  
//     return (
//     <div className=" mt-5 container" id='har'>
//     <Nav></Nav>
//     <h1 className="text-center">Shopping cart</h1>

//     <hr />
//     <div className="row d-flex m-auto">
//       <div className="col d-flex m-auto " id='form'>
//         {
//           shoppingcart1.map((item: { name: string; price: number; model: string; discount: number;count:number }) =>
//             <div className="col-3 g-3" id='f'>
//               <img src="https://www.91-img.com/pictures/143993-v4-apple-iphone-14-mobile-phone-large-1.jpg" className="img-fluid" alt="Responsive image" />
//               <Cards
//                 key={item.name}
//                 name={item.name}
//                 price={item.price}
//                 model={item.model}
//                 discount={item.discount}  

               
//               />
//                   <Link className="nav-link bg-primary"  to={"/search"}  >
//                   </Link>
            
                 
              
             
              

//             </div>
//           )

//         }
//       </div>

//       <div className="col d-flex m-auto " id='form'>
//         {
//           shoppingcart2.map((item: { name: string; price: number; model: string; discount: number; count: number }) =>
//             <div className="col-3 g-3" id='f'>
//               <img src="data:image/jpeg;base64,"  className="img-fluid" alt="Responsive image" />
//               <Cards
//                 key={item.name}
//                 name={item.name}
//                 price={item.price}
//                 model={item.model}
//                 discount={item.discount}  
             
//               />
              
             
              

//             </div>
//           )

//         }
//         {/* <NavLink className="btn btn-primary" to={"/cart"}>Add to cart</NavLink> */}
//          {/* <button className="btn btn-primary"  onClick={shoppingcart2}>Add to cart</button> */}
//       </div>


//       <div className="col d-flex m-auto " id='form'>
//         {
//           shoppingcart3.map((item: { name: string; price: number; model: string; discount: number; count:number }) =>
//             <div className="col-3 g-3"id='f'>
//               <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSZ8uCsg5XJ3d9v6-UduqkVoQmNs5u7JzmerQ&usqp=CAU" className="img-fluid"
//                alt="Responsive image"  />
//               <Cards
//                 key={item.name}
//                 name={item.name}
//                 price={item.price}
//                 model={item.model}
//                 discount={item.discount}  
//               />

              

//             </div>
//           )

//         }
//       </div>


//     </div>
//   </div>
//     );
// }

// export default Home;